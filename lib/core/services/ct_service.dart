...
class CtService {
  ...
  // ── Carnet numérique (historique par véhicule) ─────────────────────────────

  /// Retourne les véhicules CT du client (alias de getVehicles, endpoint /ct/vehicles).
  Future<List<VehicleModel>> getMyVehicles() => getVehicles();

  Future<List<VehicleInterventionModel>> getVehicleHistory(String vehicleId) async {
    final res = await _api.get('/ct/vehicles/$vehicleId/history');
    return (res.data['data'] as List)
        .map((e) => VehicleInterventionModel.fromJson(e as Map<String, dynamic>))
        .toList();
  }
